...
function App() {
  ...
  useEffect(() => {
    async function loadProfiles() {
      ...
      try {
        const response = await window.accountsManager.listProfiles();
  ...
  async function handleCreateProfile(input: ProfileFormValues): Promise<void> {
    ...
    try {
      const response = await window.accountsManager.createProfile(input);
  ...
  async function handleUpdateProfile(input: UpdateManagedProfileInput): Promise<void> {
    ...
    try {
      const response = await window.accountsManager.updateProfile(input);
  ...
  return (
    <main className="app-shell">
      <div className="workspace-header">
        <p className="eyebrow">Managed Profile Foundation</p>
        <h1>Accounts Manager</h1>
        <p className="workspace-copy">
          Create and maintain the managed profiles that will later launch isolated browser
          sessions for YouTube and Instagram accounts.
        </p>
      </div>
